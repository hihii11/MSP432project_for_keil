...
void encoder_init(Encoder_Channel_enum CHI,unsigned char MODE)
{
    if(MODE==CAPTURE)//定时器上升沿捕获
    {
       ...
       // NVIC->ISER[0] = 1 << ((TA0_N_IRQn) & 31);
        Interrupt_setPriority(INT_TA0_N, 0x20);
    }
    else//外部上升沿中断捕获
    {
...
void Encoder_clr(Encoder_Channel_enum CHI)
{
    switch(CHI)
              {
              case(ENCODER_CHA):encoder_A.encoder=0;break;
              case(ENCODER_CHB):encoder_B.encoder=0;break;
              case(ENCODER_CHC):encoder_C.encoder=0;break;
              case(ENCODER_CHD):encoder_D.encoder=0;break;
              }
}
...
void ENCODER_TEST()
{
   ...
   encoder_init(ENCODER_CHA,INT_MOD);//其他通道根据需要自行配制

   encoder_init(ENCODER_CHB,INT_MOD);
