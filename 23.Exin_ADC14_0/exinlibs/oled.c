...
//[6]0 1 2 3 ... 127
//[7]0 1 2 3 ... 127
uint8 screen_buf[128][64]={0};
unsigned char bmp[1024]={0};
unsigned char bmp2[1024]={0};
int n=0;
...
void OLED_pos_clr(unsigned char x,unsigned char y)
{
         OLED_Set_Pos(x, (unsigned char)(y >> 3));
         OLED_WR_Byte((unsigned char)(0xb0 + (y >> 3)),1);
         OLED_WR_Byte((unsigned char)(((x & 0x00) ) | 0x00),1);
}
void oled_update(char mode)
{
    ...
    if(mode==3)
    {
        for(i=0;i<128;i++)
        {
            OLED_ShowChar(i, oscill1.middle_line-Oscill_data_buff[i],' ');
        }
    }
...
void OLED_Showfloat(uint8 x, uint8 y, float din)
{
    static unsigned char tmp[8]={0};
    sprintf(tmp,"%0.2f",din);
    OLED_ShowString(x, y, tmp);
}
//显示一个字符号串
